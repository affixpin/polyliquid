  ...
  return (
    <section className="max-w-[1140px] mx-auto px-8 py-16">
      ...
      <div className="flex flex-col gap-3.5 mb-4">
        {tiers.map((t) => (
          <div key={t.name} className="grid grid-cols-[82px_48px_1fr_42px] gap-2.5 items-center">
            <span className={`font-semibold text-[14px] ${t.nameColor}`}>{t.name}</span>
            <span className="font-mono text-[12px] text-text-3 text-right">{t.count}</span>
      ...
      {/* Whale callout — full width below bars */}
